...
def build_zip_index():
    ...
    for entry in data:
        zip_filename = entry["zip_file"]
        for meta in entry["entries"]:
            fname = meta["filename"]
            if meta.get("classification") == "raw":
                raw_to_zip[fname] = zip_filename
            else:
                metadata_to_zip[fname] = zip_filename
            if "header" in meta:
            ...
            logging.debug(f"Mapped {fname} -> {zip_filename}")
    logging.info(f"Indexed {len(metadata_to_zip)} metadata files.")
    return metadata_to_zip, raw_to_zip, headers_by_file
...
def run():
    try:
        ...
        metadata_to_zip, raw_to_zip, headers_by_file = build_zip_index()
        full_summary = {}
            ...
            for dataset_name, dataset in datasets.items():
                ...
                for raw_file in raw_files:
                    ...
                    for meta in metadata_files:
                        ...
                        zip_filename = metadata_to_zip.get(meta_file)
                        zip_path = RAW_DATA_DIR / zip_filename if zip_filename else None
                    ...
                    entry = {
                        "from": rfrom,
                        "to": rto,
                        "zip_filename": raw_to_zip.get(filename),
