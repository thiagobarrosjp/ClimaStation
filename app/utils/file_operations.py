...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Testing ClimaStation File Operations")
        print("=" * 50)
        ...
        config = {
            ...
            'validation': {
                'min_measurements_per_file': 1
            }
        }
        
        # Test paths
        test_dir = Path("test_file_operations")
        test_dir.mkdir(exist_ok=True)
        
        # Test 1: Download a small test file (using a public test URL)
        print("\n1. Testing file download:")
        test_url = "https://httpbin.org/bytes/1024"  # 1KB test file
        download_path = test_dir / "test_download.bin"
        
        success = download_file(test_url, download_path, config, logger)
        if success and download_path.exists():
            print(f"   ✅ Download successful: {download_path} ({download_path.stat().st_size} bytes)")
        else:
            print("   ❌ Download failed")
        
        # Test 2: Create and test ZIP extraction
        print("\n2. Testing ZIP extraction:")
        ...
        with zipfile.ZipFile(test_zip, 'w') as zf:
            ...
            csv_content = "STATIONS_ID;MESS_DATUM;TT_10;RF_10\n1;20230101;15.5;80.2\n2;20230102;16.1;78.9\n"
            zf.writestr("test_data.txt", csv_content)
            zf.writestr("metadata.txt", "Station information file")
        
        try:
            extracted_files = extract_zip(test_zip, extract_dir, config, get_logger("EXTRACT"))
            print(f"   ✅ Extraction successful: {len(extracted_files)} files extracted")
            for file_path in extracted_files:
        ...
        if extracted_files:
            test_file = next((f for f in extracted_files if f.name == "test_data.txt"), None)
            if test_file:
                expected_cols = ["STATIONS_ID", "MESS_DATUM", "TT_10", "RF_10"]
                is_valid = validate_file_structure(test_file, expected_cols, config, get_logger("VALIDATE"))
                if is_valid:
                    print(f"   ✅ Validation successful: {test_file.name}")
                else:
                    print(f"   ❌ Validation failed: {test_file.name}")
            else:
                print("   ⚠️  Test data file not found in extraction")
        else:
            print("   ⚠️  No extracted files to validate")
        ...
        bad_download = download_file(bad_url, test_dir / "nonexistent.txt", config, logger)
        print(f"   ✅ Bad download handled correctly: {not bad_download}")
        
        # Test validation of non-existent file
        bad_validation = validate_file_structure(Path("nonexistent.txt"), ["COL1"], config, get_logger("VALIDATE"))
        print(f"   ✅ Bad validation handled correctly: {not bad_validation}")
